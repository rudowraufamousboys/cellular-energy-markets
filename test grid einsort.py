...
for z in range (0,96):
    ...
    SupplyB1B2A1accdfi=SupplyB1B2A1dfi.iloc[1:].cumsum(axis=1, skipna=True)
    ...
    linecap=200
                                                             ...
                                                             # name of the cell
    x=SupplyB1B2A1accdfi.iloc[:,0]                                             # value of x axis of the plot is accumulated price of Energy
    x1=SupplyB1B2A1accdfi.iloc[:,1]
    x2=SupplyB1B2A1accdfi.iloc[:,2]
    x3=SupplyB1B2A1accdfi.iloc[:,3]
    x4=SupplyB1B2A1accdfi.iloc[:,4]
    
    y=SupplyB1B2A1dfi.iloc[0,0]
    y1=SupplyB1B2A1dfi.iloc[0,1]
    y2=SupplyB1B2A1dfi.iloc[0,2]
    y3=SupplyB1B2A1dfi.iloc[0,3]
    y4=SupplyB1B2A1dfi.iloc[0,4]

    
    D=SumloadB1B2A1df.iloc[:,0]  
                                            # loop for plots
    Sxi=[0,0,x[z],x[z],x1[z],x1[z],x2[z],x2[z],x3[z],x3[z],x4[z]]
    Syi=[0,y,y,y1,y1,y2,y2,y3,y3,y4,y4]
    #Gxi=[x1[z],x1[z],x1[z]+linecap]
    #Gyi=[y1[0],15,15]
    Dxi=[D[z],D[z]]
    Dyi=[0,y4*2]
    ...
    plt.title(name+' '+ SupplyB1B2A1accdfi.index[z])
    #plt.axis([0,300, \
    #          0,y1*1.2])
    plt.show()
