...
for z in range(SupplyB1B2A1df.index.size-1):
        ...
        for Z in range (1,len(x_values)):
            ...
            if x_values[-Z]>x_me:
                ...
                print(SupplyB1B2A1accdfi.columns[-Z]+' wird abgeregelt')
                
                if (x_values[-Z]-x_values[-Z-1])<(x_values[-Z]-x_me):
                    print(x_values[-Z]-x_values[-Z-1])
                else:
                    print(x_values[-Z]-x_me)
        
                Energycut.append(SupplyB1B2A1accdfi.columns[-Z]+' wird abgeregelt')  
        
        
            
        if x_values[0]>x_me:
                
            print(SupplyB1B2A1accdfi.columns[0]+' wird abgeregelt')  
        
            Energycut.append(SupplyB1B2A1accdfi.columns[0]+' wird abgeregelt')
                
            print(x_values[0]-x_me)
    
                    
            
        #print(SupplyB1B2A1accdfi.columns[-Z-1])
                
        #print(x_values[-Z-1]-x_me)
